...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(TCRT_DROIT), rightLineInteruption, HIGH);  // Attache l'interruption du capteur ligne blanche au pin TCRT_DROIT
  attachInterrupt(digitalPinToInterrupt(TCRT_GAUCHE), leftLineInteruption, HIGH); // Idem avec TCRT_GAUCHE
...
/*
 * Vérifie si le capteur de ligne gauche a detecté une ligne (value=HIGH)
 * Si c'est le cas, il repart dans la direction opposée à la ligne
 * pendant une demi seconde, puis s'arrête
 */
void leftLineInteruption(){
  Serial.println("[ESIEESPACE] Ligne blanche à gauche !");
...
/*
 * Vérifie si le capteur de ligne droit a detecté une ligne (value=HIGH)
 * Si c'est le cas, il repart dans la direction opposée à la ligne
 * pendant une demi seconde, puis s'arrête
 */
void rightLineInteruption(){
  Serial.println("[ESIEESPACE] Ligne blanche à droite !");
